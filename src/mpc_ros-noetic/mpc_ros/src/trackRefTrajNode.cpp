...
MPCNode::MPCNode()
{
    ...
    _sub_odom   = _nh.subscribe("gazebo/ground_truth/state", 1, &MPCNode::odomCB, this);
    _sub_path   = _nh.subscribe( "/me5413_world/planning/local_path", 1, &MPCNode::pathCB, this);
    _sub_gen_path   = _nh.subscribe( "/me5413_world/planning/local_path", 1, &MPCNode::desiredPathCB, this);
    ...
    if(_pub_twist_flag)
        _pub_twist = _nh.advertise<geometry_msgs::Twist>("/jackal_velocity_controller/cmd_vel", 1); //for stage (Ackermann msg non-supported)
...
//            }
        ...
        for(int i = min_idx; i < N ; i++)
        {
            if(total_length > _pathLength)
                break;
            geometry_msgs::PoseStamped modifiablePose = totalPathMsg->poses[i];
            modifiablePose.header.frame_id = "odom";
            modifiablePose.header.seq = i;
            modifiablePose.header.stamp = ros::Time::now();
            cout << modifiablePose << endl;
            _tf_listener.transformPose(_odom_frame, ros::Time(0) , 
                                            modifiablePose, _odom_frame, tempPose);
...
{          
    ...
    if(_pub_twist_flag)
    {
        ...
        cout << "x: " << _twist_msg.linear.x << endl;
        cout << "z: " << _twist_msg.angular.z << endl;
